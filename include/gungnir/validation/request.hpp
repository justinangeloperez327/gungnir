...
public:
    explicit ValidatedRequest(const http::Request& request) : request_(&request) {}
    virtual ~ValidatedRequest() = default;

    [[nodiscard]] Input validated() const {
        return Validator::validate(request_->all(), rules());
    }

    [[nodiscard]] const http::Request& request() const noexcept { return *request_; }
